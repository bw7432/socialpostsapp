class V1::EventsController < ApplicationController
  ...
  def index
    @events = Event.order(created_at: 'desc').paginate(:page => params[:page], :per_page => 25)
    render json: @events
  end
  ...
  param :id, :number
  def user_events
    @events = Event.where(user_id: params[:id]).order(created_at: 'desc').paginate(:page => params[:page], :per_page => 25)
    render json: @events
  end
